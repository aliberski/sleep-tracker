...
describe('Auth tabs', () => {
  ...
  it('should fill and send the register form', async () => {
    await element(by.id(auth.register.inputEmail)).replaceText("test@email.pl");
    await element(by.id(auth.register.inputLogin)).replaceText("testowyLogin123");
    await element(by.id(auth.register.inputPassword)).replaceText("test123");
  ...
  it('should fill and send the remind password form', async () => {
    await element(by.id(auth.remindPassword.inputEmail)).replaceText("test@email.pl");
    await element(by.id(auth.remindPassword.buttonBack)).tap();
    await expect(element(by.id(auth.login.id))).toBeVisible();
  });

  it('should fill and send the login form', async () => {
    await element(by.id(auth.login.inputLogin)).replaceText("test@email.pl");
    await element(by.id(auth.login.inputPassword)).replaceText("testowyLogin123");
